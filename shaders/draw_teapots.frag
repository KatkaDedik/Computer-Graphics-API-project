...
flat layout(location = 2) in int fs_id;
...
void main()
{
	...
	for(int i = 0; i < 1; i++) {
		...
		vec3 ambient = objects[fs_id].ambient_color.rgb * light.ambient_color.rgb;
		vec3 diffuse = objects[fs_id].diffuse_color.rgb * light.diffuse_color.rgb;
		vec3 specular = objects[fs_id].specular_color.rgb * light.specular_color.rgb;

		vec3 color = ambient.rgb
			+ NdotL * diffuse.rgb
			+ pow(NdotH, objects[fs_id].specular_color.w) * specular;
		color /= distance2;
